...
def make_batch_excel_bytes(dataframe):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        # Dashboard title.
        dashboard_sheet.merge_cells("A1:L2")
        dashboard_sheet["A1"] = "Student Performance Prediction Report"
        dashboard_sheet["A1"].fill = navy_fill
        ...
        dashboard_sheet.row_dimensions[1].height = 28
        dashboard_sheet.row_dimensions[2].height = 12

        dashboard_sheet.merge_cells("A3:L3")
        dashboard_sheet["A3"] = (
            "Batch prediction dashboard generated by the Student AI system"
        )
        dashboard_sheet["A3"].fill = light_purple_fill
        ...
        kpi_cards = [
            ("A5:C7", "Total Students", total_students, "1E3A8A", "FFFFFF"),
            ("D5:F7", "Excellent", int(counts["Excellent"]), "DCFCE7", "166534"),
            ("G5:I7", "Good", int(counts["Good"]), "DBEAFE", "1D4ED8"),
            ("J5:L7", "At Risk", int(counts["At Risk"]), "FEE2E2", "B91C1C"),
        ]

        for cell_range, label, value, fill_color, font_color in kpi_cards:
            dashboard_sheet.merge_cells(cell_range)
            start_cell = cell_range.split(":")[0]
            dashboard_sheet[start_cell] = f"{label}\n{value:,}"
            dashboard_sheet[start_cell].fill = PatternFill(
                "solid",
                fgColor=fill_color,
            )
        ...
        # Dashboard source table for charts.
        dashboard_sheet["A10"] = "Performance Category"
        dashboard_sheet["B10"] = "Students"
        dashboard_sheet["C10"] = "Percentage"

        for cell in dashboard_sheet["10:10"][:3]:
