...
page = st.sidebar.radio("Menu", ["Home", "Prediction", "Model Results", "Dataset", "About"])

st.markdown(
    '<div class="hero"><h1>Student Performance Prediction</h1>'
    '<p>Supervised classification using KNN, SVM and ANN.</p></div>',
    unsafe_allow_html=True
)
...
if page == "Home":
    ...
    st.plotly_chart(
        px.bar(chart, x="Model", y="Score", color="Metric", barmode="group", range_y=[0, 1]),
    ...
    with st.form("prediction_form"):
        ...
        number_of_subjects = st.slider("Number of Subjects", 1, 12, 5)
        scores = []
        ...
        study_hours = st.slider("Study Hours Per Day", 0.0, 12.0, 3.0, 0.1)
        previous_cgpa = st.slider("Previous CGPA", 0.0, 4.0, 3.0, 0.01)
    ...
    if submit:
        ...
        st.success(f"Prediction: {best_row['Prediction']}")
        st.metric("Confidence", f"{best_row['Confidence']:.1%}")
        st.dataframe(
            result_df.assign(Confidence=result_df["Confidence"].map(lambda x: f"{x:.1%}")),
            hide_index=True,
            use_container_width=True
        )
    ...
    selected = st.selectbox("Select Model", ["ANN", "SVM", "KNN"])
    st.image(
    ...
    with c1:
        st.plotly_chart(
            px.histogram(df, x="Average_Score", nbins=25, title="Average Score Distribution"),
            use_container_width=True
        )
    with c2:
        st.plotly_chart(
            px.scatter(
                df, x="Attendance_Pct", y="Average_Score",
    ...
    st.subheader("About")
    st.markdown("""
This project predicts student performance using supervised machine learning.
...
The original Kaggle dataset did not include subject-level marks. Therefore,
Number of Subjects and Average Score are transparently added as reproducible
engineered estimates for this project.
""")
